...
const NavSignOut = (props: {name: string | null, setCurrentUser: Dispatch<SetStateAction<User | undefined>>;}) => {
  ...
  return (
    <button
      ...
      onClick={() => {
        signOut(auth).then(() => {auth.updateCurrentUser(null);props.setCurrentUser(undefined);});
        router.push("/")
      }}
    >
      {loading == true ? (
